...
GSY.gamebox.playGame = function(element,index){
	...
	if(_data.gamecode == 'fbtr') data['gametype'] = 1;
...
GSY.gamebox.getGameHistoryPlayGame = function(element){
	...
	if(_data['url'].indexOf('?')) var url = _data['url'] + '&leftbar_collapse=yes';
	else var url = _data['url'] + '?leftbar_collapse=yes';
	_data['url'] = url;
	if(data.gameid == 'fbtr') _data['gametype'] = 1;
...
;(function(){
	...
	var handle = function(element,data,listData,gamecode){
		...
		element.click(function(){
			...
			//开启游戏界面传输参数设置
			if(data['url'].indexOf('?')) var url = data['url'] + '&leftbar_collapse=yes';
			...
			if(gamecode == 'fbtr') openGameData['gametype'] = 1;
 ...
 */
;(function(){
	if(typeof oasGAME_RIGHT_ADS == 'undefined') return; 
...
$(function(){
	...
	//设置用户名
	var fullname = GSY.util.getUrlParam('name');
	var name = GSY.gamebox.strCut(fullname,16,true);
	$('#gb-gamebox-userinfo-name').html(name).parent().attr('title',fullname);
