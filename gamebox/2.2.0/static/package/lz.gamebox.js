...
;(function(){
	...
	resize.prototype = {
		...
		inti : function(){
			var that = this;
			this.scroll = CXGResetScroll({
				element : $('.lz-main'),
				scrollWrapWidth : 16,
				scrollWidth : 14,
				scrollToBottomNum:100,
				scrollToBottomCallback:function(){
					that.getData();
				}
			});	
		...
		setPosition : function(){
			...
			/*GSY.gamebox.message($(window).width())*/
			this.scroll.setScrollEleCss();
		},
	...
	/**
	 * [createObj description] 游戏列表对象实例化
...
;(function(){
	...
	history.prototype = {
		...
		getGameList : function(){
			...
			if(games){
				arr = games.split(this.options.splitFile);
			}
			return arr;
		},
		...
		show : function(x){
			...
			GSY.config.scrollCode['oas_menu_box_history_left'] = function(){
				...
				box.find('.lz-menu-icon').css({right:x-32});				
			};
	...
	GSY.history.set = function(str){
		if(!lastObj){
			setTimeout(function(){GSY.history.set(str)},500);
		} 
		else{
			lastObj.setData(str);
		}
...
;(function(){
	...
	search.prototype = {
		...
		init : function(ret){
			...
			var key = '';
			if(ret.keyword!=''&&ret.keyword.length>0){
				key = Base64.decode(ret.keyword);
			}	
			var x = '';		
			if(ret.pos!=''&&ret.pos.length>0){
				x = parseInt(Base64.decode(ret.pos))
			}			
			this.PageX = x ? x : 80;
