...
GSY.gamebox.getServerList = function(uid,data,token){
	...
	GSY.util.ajax({
		...
		success : function(ret){
			...
			GSY.gamebox.getLastServerListCode(ret.played?ret.played:null);	
...
GSY.gamebox.toPlayGame = function(element){
	...
	if(urlStr.indexOf('?')) var url = urlStr + '&leftbar_collapse=yes';
...
GSY.gamebox.getAllServerListCode = function(data){
	...
	for(var i = 0 ; i < len ; i++){
		var temp = data[i];
		if(temp.recommand == 1){
			recommandArr.push(temp);	
		}
	}

	var temp = recommandArr[recommandArr.length-1];
	var post = GSY.gamebox.jsonToPostStr(temp);
	if(temp.url.slice(0, 5) == 'http:') {
		var url = temp.url;
	}else{
		var url = 'http:' + temp.url;
	} 
