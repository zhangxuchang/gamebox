...
GSY.gamebox.playGame = function(gameData){
	...
	var handle = function(ret){
		...
		if(ret.played && ret.played.length > 0){
			...
			url = ret.played[0].url.indexOf('?') != -1 ? ret.played[0].url + '&leftbar_collapse=yes&logintype=4' : ret.played[0].url + '?leftbar_collapse=yes&logintype=4';
			//增加可扩展参数
		...
		if(recommandArr.length == 0){
			GSY.gamebox.message(OASGetLangVal('msg_norecommandserver'));
			return;		
		};
		...
		url = item.url.indexOf('?') != -1 ? item.url + '&leftbar_collapse=yes&logintype=4' : item.url + '?leftbar_collapse=yes&logintype=4';
