...
GSY.gamebox.initNewDate = function(parentElement){
	...
	GSY.util.ajax({
		...
		success : function(ret){
			var html = '';
			GSY.util.echo(JSON.stringify(ret),'初始化登陆页面最新新闻数据');
			if(ret && ret.code == 1){
	...
	GSY.util.ajax({
		...
		success : function(ret){
			...
			if(ret.status == 'ok'){
				...
				if(usernameTip.length==0||passwordTip.length==0||accpassError.length==0){
...
GSY.gamebox.oneClickGoPlayGame = function(element){
	...
	if(count == 0){
		...
		//GSY.gamebox.message('未选登陆账号');//LANG[GSY.config.get('lang')]['Gamebox_login_fail']
	}
}
...
GSY.gamebox.oneClickGoPlayGameCode = function(item){
	...
	//判断用户是否登陆
	var flag = GSY.gamebox.userRunCodeStatus(Base64.decode(_data.username));
	if(flag) return;
	...
	GSY.gamebox.getUserToken(function(token,uid){
		...
		//js调用c++方法
		GSY.gamebox.cAjax('WINIE_LoginOK',data,'用户开小号登陆成功');
	},{email : Base64.decode(_data.username) , pwd : Base64.decode(_data.pwd)});
};
...
GSY.gamebox.FBloginCallback = function(ret){
	...
	if(ret.status == 'ok') {
		var data = {
			...
			type : 'fb',
			username : Base64.encode(ret.val.uname),
			pwd : 'false',
			remember : 'false',
			automatic : 'false',
			uid: ret.val.id,
...
GSY.gamebox.loginCallback = function(ret){
	...
	if (ret.code == 1) {
		if (GSY.gamebox.loginMainType === 'play') {  //登录完成跳转到游戏页面
...
GSY.gamebox.setUserData = function(ret){
	...
	//var usernameTip = item.find('')
	
	//初始化是否记住密码
	if(ret.remember == 'true'){
		rememberElement.attr('checked','checked');
		//初始化用户密码
		username.val(Base64.decode(ret.username));

		//在密码框上定位关闭，然后可以使用回车键事件
		pwd.focus();
		pwd.val(Base64.decode(ret.pwd));
	}else{		
...
GSY.gamebox.initUserListData = function(ret,fullname,url){
	...
	GSY.util.echo(fullname,'创建账号多->游戏名称');
	GSY.util.echo(url,'创建账号多->游戏地址');
	...
	//设置默认昵称
	var mainAccount = Base64.encode(LANG[GSY.config.get('lang')]['Gamebox_mainAccount']);
	...
	for(var i = 0 ; i < len ; i++){
		...
		//账号全名称
		tempStr = tempStr.replace('\{gameContent\}',GSY.str.formatStr(GSY.str.cut(Base64.decode(ret[i].username),24,true)));

		//账号缩略名称
		tempStr = tempStr.replace('\{gameFullContent\}',GSY.str.formatStr(Base64.decode(ret[i].username)));

		//该账号最后玩过的区服信息
		tempStr = tempStr.replace('\{gameLastServer\}',fullname);//Base64.decode(fullname)

		//用户缩略昵称
		var desc = GSY.str.cut(Base64.decode(descStr),10,true);
		tempStr = tempStr.replace(/\{gameDesc\}/g,GSY.str.formatStr((desc))); 

		//用户全昵称
		tempStr = tempStr.replace(/\{gameFullDesc\}/g,GSY.str.formatStr(Base64.decode(descStr)));//Base64.decode(descStr)
...
GSY.gamebox.userRuning = function(ret){
	...
	for(var key in ret){
		if (key > 0) {
			var tempstr = Base64.decode(ret[key]['username']) + '=' + ret[key]['gameid'] + '=' + ret[key]['fullname'];	
			runing.push(tempstr);
		};	
	}
...
GSY.gamebox.delUserList = function(element,event){
	...
	var name = data.discription_ != 'null' ? Base64.decode(data.discription_) : LANG[GSY.config.get('lang')]['Gamebox_mainAccount'];
...
GSY.gamebox.changeDiscrition = function(element,event){
	...
	GSY.gamebox.changeDiscritionCallback = function(ret){
		...
		if(ret.code == 1 ){
			...
			data['discription_'] = Base64.encode(strVal);
...
GSY.gamebox.changeDiscritionCode = function(input,data){
	...
	data['discription_'] = Base64.encode(input.val());
	var str = GSY.gamebox.jsonTojsonStr(data);
	//js调用c++方法
