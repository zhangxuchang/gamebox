...
GSY.gamebox.getUserToken = function(callback, data, element) {
    ...
    //登陆用户中心
    GSY.util.ajax({
        url: '//passport.oasgames.com/index.php?m=mynetAPILogin&access_token=' + data,
        type: 'get',
        success: function(ret) {
            ...
            if (ret.status == 'ok') {
                ...
                GSY.gamebox.getLoginUserId(ret.val.loginKey, GSY.config.get('gamecode'), function(uid) {
...
/**
 * [getUserToken description] 获取mynet用户登陆token
 * @param  {Function} callback [description]
 * @param  {[type]}   options  [description]
 * @return {[type]}            [description]
 */
GSY.gamebox.getMynetUserToken = function(data, element) {
    ...
    //登陆用户中心
    GSY.util.ajax({
        url: '//uyeler.mynet.com/index/loginjsonp.jsp?callback=?',
        data: data,
        type: 'get',
        success: function(ret) {
            //GSY.util.echo(ret.val);
            ...
            if (ret.result == '1') {
                ...
                GSY.gamebox.getUserToken(function(token, uid) {
                    ...
                    //js调用c++方法;
                    GSY.gamebox.cAjaxCallback('WINIE_LoginOK', data, 'mynet用户登陆成功')

                },ret.encrypted, element);
                
            } else if (ret.result == '-1') { //登陆失败处理
                accountError.html('');
                loginstatus.html('');
                accpassError.html(OASGetLangVal('Gamebox_AccPassError'))
...
GSY.gamebox.login = function(element) {
    ...
    //获取mynet用户token,调用oas接口
    GSY.gamebox.getMynetUserToken({
        u: usernameval,
        p: passwordval
    }, element);
