...
if(tiyanversionInfo && tiyanversionInfo.openGamesFlag === 1){ 
...
GSY.gamebox.playGame = function(element,index){
    ...
    //window.console.info(GSY.gamebox.GameListData);
    //alert(obj.gamecode);

	var url = GSY.config.get('path').replace(/\{pagename\}/,'game')+ '?' + GSY.gamebox.jsonToPostStr(param);
	var data = {
		service_area : obj.name,
		url : GSY.gamebox.getGameUrl(url,obj.gamecode),
		gameid : obj.gamecode,
		language : GSY.config.get('lang'),
		type : 'game',
		playtype : GSY.gamebox.GameListData[obj.gamecode].playtype,
		gametype : 'login'
	}

	GSY.gamebox.consoleInfo(data);
    ...
    //
    GSY.gamebox.savePlayLog(obj.gamecode);
...
GSY.gamebox.fromGameHistoryPlayGame = function(element){
	...
	if(param['url'].indexOf('?')) var url = param['url'] + '&leftbar_collapse=yes';
	else var url = param['url'] + '?leftbar_collapse=yes';
	//增加可扩展参数
	url += GSY.gamebox.getOtherParam(data.gameid);

	param['url']      = GSY.gamebox.getGameUrl(url,data.gameid);
	param['gameType'] = 'login';
	param['playtype'] = GSY.gamebox.GameListData[data.gameid].playtype;

	if(GSY.str.cut(data.gameid,2) == 'le') param['gameType'] = 'u3d';

	GSY.gamebox.consoleInfo(param);
...
GSY.gamebox.createRecommendServerCode = function(data,recommendGameData,box,index,islast){
	...
	box.click(function(){
		...
		//用户已经登录
			...
			openGameData['language']     = GSY.config.get('lang');
			openGameData['url']          = GSY.gamebox.getGameUrl(url,data.gamecode);
			openGameData['service_area'] = data.name;
			openGameData['gameType']     = 'login';
			openGameData['playtype']     = GSY.gamebox.GameListData[data.gamecode].playtype;
			//设置特殊游戏类型
			if(GSY.str.cut(data.gamecode,2) == 'le'){
				openGameData['gameType'] = 'u3d';
			}

			GSY.gamebox.consoleInfo(openGameData);
    		//GSY.gamebox.cAjax('WINIE_SetGameMsg',historyData,'设置热门游戏打开游戏存储记录');
			//GSY.gamebox.cAjax('WINIE_MainUserOpenGame',openGameData,'从热门游戏打开游戏');
...
GSY.gamebox.getGamesData = function(){
	...
	GSY.util.ajax({
		...
		success : function(ret){
			...
			GSY.gamebox.initGameListData(ret);
...
// 获取游戏 playtype 信息
GSY.gamebox.initGamaData = function(gameInfo){
	...
	if(gameInfo.url.indexOf('.exe') != -1)
	{
		gameInfo['playtype'] = 'exe';
	}
	else
	{
		gameInfo['playtype'] = 'flash';
	}
...
// 存储数据
GSY.gamebox.GameListData = {};
GSY.gamebox.GameBoxGlobal = {};

// 按照指定的格式存储游戏列表数据
GSY.gamebox.initGameListData = function(ret){

    //GSY.gamebox.showSysInitDebugMsg();
	...
	for(var key in ret.data.html)
	{
		var gameList = ret.data.html[key];

		for(var key2 in gameList)
		{
			var gamecode = gameList[key2]['gamecode'];
			if(gamecode != null && GSY.gamebox.GameListData[gamecode] == null)
			{
				GSY.gamebox.GameListData[gamecode] = GSY.gamebox.initGamaData(gameList[key2]);
			}
		}
	}

	GSY.gamebox.consoleInfo(GSY.gamebox.GameListData);

    //
    GSY.gamebox.autoStartGame();
}

// 按照指定格式管理游戏url
GSY.gamebox.getGameUrl = function(url,gamecode){

    if(!GSY.gamebox.GameListData[gamecode])
    {
        return url;
    }

	var playtype = GSY.gamebox.GameListData[gamecode].playtype;

	if(playtype == 'exe')
	{
		url = GSY.gamebox.GameListData[gamecode].url;
	}
...
// 控制台输出
GSY.gamebox.consoleInfo = function(obj){
...
// GSY.gamebox.showSysInitDebugMsg = function(obj){

//     //switch
//     return false;
//     //
//     alert('[Debug]:welcome to OASIS Gamebox');
// }

//登录版或者体验版登录完成打开游戏
GSY.gamebox.autoStartGame = function(gamecode){

    var gamecode   = GSY.gamebox.GameBoxGlobal['first_start_gamecode'];
    var token      = GSY.gamebox.GameBoxGlobal['token'];
    var playertype = GSY.gamebox.GameBoxGlobal['playertype'];
...
// 记录exe游戏打开log
GSY.gamebox.savePlayLog = function(gamecode){

    var gameInfo = GSY.gamebox.GameListData[gamecode];

    if(gameInfo == null || typeof(gameInfo) == 'undefined')
    {
        return;
    }

    if(gameInfo.playtype != 'exe')
    {
        return;
    }
    ...
    data['gameid']   = gamecode;
    data['language'] = gameInfo.lang;
    data['name']     = gameInfo.name;
    data['ico']      = gameInfo.ico;

    data['server_sid']  = 1;
    data['server_type'] = 1;
    data['server_prex'] = '';
    data['valid']       = '1';
    data['server_name'] = gameInfo.name;
    data['fullname']    = gameInfo.name;
    data['recommand']   = '1';
    data['url']         = gameInfo.url;
    data['merger']      = '0';
    data['start_time']  = '0';

    GSY.gamebox.cAjaxCallback('WINIE_SetGameMsg',data,'记录exe游戏，玩游戏日志');
...
$(function(){
	...
	GSY.gamebox.GameListData[gamecode] = {
		playtype:'flash'
	}	
    GSY.gamebox.GameBoxGlobal['first_start_gamecode'] = gamecode;
    GSY.gamebox.GameBoxGlobal['token']                = token;
    GSY.gamebox.GameBoxGlobal['playertype']           = playertype;
	GSY.gamebox.mainUserId = '000000000000000';
	

	//如果是体验版本
	if(playertype == 'tiyanversion'){
		var tiyanCallback = function(ret, freeuser){
			//GSY.gamebox.message(ret.val.loginKey);
			GSY.gamebox.mainUserId = ret.val.id;
			var type = tiyanversionInfo.openGamesFlag == 1 ? 'play' : 'main';		
			//GSY.gamebox.message('tiyanCallback');	
			GSY.gamebox.cAjax('WINIE_UpdataTokenOfRemote',{token:ret.val.loginKey,uid:ret.val.id,type:type},'储存试玩token');	
			//GSY.gamebox.message(tiyanversionInfo.openGamesFlag);
			
			if(tiyanversionInfo.openGamesFlag == 1){
				//GSY.gamebox.message('openGamesFlag');	
				GSY.gamebox.cAjax('WINIE_LoginOK',{token:ret.val.loginKey,username: ret.val.uname,uid:ret.val.id,type:'oas', freeuser:freeuser});
				//GSY.gamebox.message(ret.val.loginKey);
			    var param = {
		...
		if(token && token.length){
			GSY.gamebox.getUserData(token,function(ret){
				if(ret.val.uname.indexOf('@oas_auto') == -1){ 
					tiyanCallback(ret, false);
				}else{ 
					tiyanCallback(ret, true);
				}
			},true);
		}else{
	...
	//登录版或者体验版登录完成打开游戏
    // 代码启动方式改变了  -- by zhangxuchang 2016.01.20
