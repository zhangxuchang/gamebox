...
GSY.gamebox.createRecommendServerCode = function(data,recommendGameData,box,index){
	...
	//被截取服务器名称
	html = html.replace(/\{showServername\}/g,GSY.gamebox.strCut(recommendGameData.server_name,7,true));
	...
	box.click(function(){
		...
		//用户已经登录
			...
			//开启游戏界面传输参数设置
			if(data['url'].indexOf('?')) var url = recommendGameData['url'] + '&leftbar_collapse=yes';
...
GSY.gamebox.getGamesData = function(){
	...
	GSY.util.ajax({
		...
		success : function(ret){
			...
			//创建小游戏列表
			//波语
			if(ret.data.lang=='pl'|| ret.data.lang=='es' || ret.data.lang=='de' || ret.data.lang=='el' || ret.data.lang=='sv' || ret.data.lang=='nl'){
				$('.gb-mingame-list').hide();
				$('.gb-gameActive-box').css('background','#f1f1f1')
			}else{
				$('.gb-mingame-list').show();
				$('.gb-gameActive-box').css('background','#e4e4e4')
...
GSY.gamebox.createMiniGameLists = function(retArr){
	...
	for(var i=0,len=retArr.length;i<len;i++){
		...
		//游戏链接
		temp = temp.replace(/\{url\}/g,data.url.replace('/gamebox/','/'+GSY.config.get('file')+'/gamebox/'));
