...
GSY.config.set('version',typeof JS_CONFIG == 'undefined' || typeof JS_CONFIG.version == 'undefined' ? new Date().getTime() : JS_CONFIG.version);
GSY.config.set('lang', typeof JS_CONFIG == 'undefined' || typeof JS_CONFIG.lang == 'undefined' ? 'cn' : JS_CONFIG.lang);
...
/**
 * [getEleData description] 解析属性特定字符串，并转换为特定对象格式返回
 * @param  {[object]} element [description]  对象
 * @param  {[string]} attr    [description]	 元素属性名称
 * @return {[object]}         [description]  返回json格式
 */
GSY.util.urlToJson = function(str){
	var json = {} , temp = [] , len , klen;
	if(!str) return {};
	temp = str.split('&');
	len = temp.length;
