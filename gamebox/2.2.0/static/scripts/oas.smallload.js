	...
	var handle = function(ret){
		...
		if (GSY.gamebox.activeGames.ismain === 'true') {
			...
			if (ret.played && ret.played.length > 0) {
				...
				//判断链接地址中有参数
				if (ret.played[0].url.indexOf('?') != -1) url = ret.played[0].url + '&leftbar_collapse=yes&logintype=4';
				//判断链接地址中无参数
				else url = ret.played[0].url + '?leftbar_collapse=yes&logintype=4';
				//增加可扩展参数
...
GSY.gamebox.getXiaoUrl = function(json){
	...
	var urlStr = json.data,
		url = urlStr.indexOf('?') != -1 ? urlStr + '&leftbar_collapse=yes&logintype=4' : urlStr + '?leftbar_collapse=yes&logintype=4';
...
$(function(){
	...
	GSY.gamebox.getLocalGameData({key:'OAS_GAME_INFRO_' + gamecode},function(ret){
		...
		//登陆游戏成功之后打开游戏操作
		GSY.gamebox.playGame({name:data.name,gamecode:gamecode});
	});
});
