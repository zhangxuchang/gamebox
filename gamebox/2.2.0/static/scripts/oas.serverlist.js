...
GSY.gamebox.toPlayGame = function(element){
	...
	if(urlStr.indexOf('?') != -1) var url = urlStr + '&leftbar_collapse=yes&logintype=4';
	else var url = urlStr + '?leftbar_collapse=yes&logintype=4';
	//增加可扩展参数
...
GSY.gamebox.setBodyBg = function(bg){
	...
	var version = GSY.config.get('version');

	//背景图片为空不处理
	if(typeof bg != 'undefined'){
		main.css('background','url("'+ bg +'?ver='+ version +'") no-repeat top left');	
	}
}
