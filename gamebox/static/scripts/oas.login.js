...
;(function(){
	if(pageType == 'login'){
		...
		var pwd = login.find('input[data-name=password]');
...
GSY.gamebox.loginCallback = function(ret){
	...
	//GSY.util.echo(['loginCallback222=>' + ret]);
	if(typeof ret == 'string') ret = GSY.gamebox.eval(ret);
	
	if(ret.code == 1){
		var url = GSY.config.get('path').replace(/\{pagename\}/,'main') + '?token=' + ret.token + '&lg=' + GSY.config.get('lang') + '&gameid=' + GSY.config.get('gamecode') + '&name=' + ret.name +'&uid=' + GSY.gamebox.getUserUid;
...
GSY.gamebox.initUserListData = function(ret,fullname,url){
	...
	for(var i = 0 ; i < len ; i++){
		...
		var desc = GSY.str.cut(descStr,14,true);
...
GSY.gamebox.changeDiscrition = function(element,event){
	...
	GSY.gamebox.changeDiscritionCallback = function(ret){
		...
		if(ret.code == 1 ){
			...
			var str = GSY.str.cut(strVal,14,true);
			em.html(str).show();
