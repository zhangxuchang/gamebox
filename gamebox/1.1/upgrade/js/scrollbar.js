...
duma.BeautifyScrollBar = function(obj,arrowUpCss,arrowUpActiveCss,handleCss,handleActiveCss,arrowDownCss,arrowDownActiveCss) { 
   ...
   // this.obj.innerHTML = '<div id="' + obj + 'Area" class="dumascroll_area">' + this.obj.innerHTML + '</div><div id="' + obj + 'Bar" class="dumascroll_bar"><div class="dumascroll_arrow_up"></div><div class="dumascroll_handle"></div><div class="dumascroll_arrow_down"></div></div>'; 
    this.area = duma.$(obj + "_Area"); 
    this.bar = duma.$(obj + "_Bar"); 
	this.barHeight = this.area.offsetHeight - 10;
    this.barPos; 
    ...
    this.handleHeight = parseInt(this.area.offsetHeight/this.area.scrollHeight * (this.barHeight - this.arrowUpHeight - this.arrowDownHeight)); 
} 
duma.BeautifyScrollBar.prototype = { 
    ...
    sethandle:function() {                //当内容超多时设置拖拽条子的最小高�?
        this.handle.style.top = this.arrowUpHeight + "px"; 
        if(this.handleHeight > this.handleMinHeight) { 
            this.handleHeight < this.barHeight - this.arrowUpHeight - this.arrowDownHeight ? this.handle.style.height = this.handleHeight + "px" : this.handle.style.height = "0px"; 
        } 
        else { 
    ...
    areaScroll:function() { 
        this.handle.style.display != "none" ? this.handle.style.top = this.area.scrollTop/(this.area.scrollHeight - this.area.offsetHeight) * (this.barHeight - this.handleHeight - this.arrowUpHeight - this.arrowDownHeight) + this.arrowUpHeight + "px" : null; 
    }, 
    ...
    handleMove:function(e) { 
        ...
        this.area.scrollTop = (e.clientY - this.relY - this.arrowUpHeight)/(this.barHeight - this.handleHeight - this.arrowUpHeight - this.arrowDownHeight)*(this.area.scrollHeight - this.area.offsetHeight); 
        this.setBarPos(); 
    }, 
    ...
    barScroll:function(e) { 
        var e = e || window.event,eDir;        //设置滚轮事件,e.wheelDelta与e.detail分别兼容IE、W3C，根据返回值的正负来判断滚动方�?
        if(e.wheelDelta) { eDir = e.wheelDelta/120; } 
        else if(e.detail) { eDir = -e.detail/3; } 
        eDir > 0 ? this.area.scrollTop -= 80 : this.area.scrollTop += 80;    //步长�?0像素比较接近window滚动条的滚动速度 
...
duma.BeautifyScrollBar.init = function() { 
	...
	new duma.BeautifyScrollBar("dumaScrollAreaId","dumascroll_arrow_up","dumascroll_arrow_up_a","dumascroll_handle","dumascroll_handle_a","dumascroll_arrow_down","dumascroll_arrow_down_a").run(); 

	
	$("#dumaScrollAreaId").mouseover(function(){
		if($(".dumascroll_handle").height()>0){
			$(".dumascroll_bar").show();
		}else{
			$(".dumascroll_bar").hide();
		}
		/*$(".dumascroll_arrow_up").addClass("dumascroll_arrow_up_a");
		$(".dumascroll_handle").addClass("dumascroll_handle_a");
		$(".dumascroll_arrow_down").addClass("dumascroll_arrow_down_a");*/
	}).mouseout(function(){
		$(".dumascroll_bar").hide();
		/*$(".dumascroll_arrow_up").removeClass("dumascroll_arrow_up_a");
		$(".dumascroll_handle").removeClass("dumascroll_handle_a");
		$(".dumascroll_arrow_down").removeClass("dumascroll_arrow_down_a");*/
	});
} 
